...
def Index(request):
    ...
    if request.GET.get('order') == 'total_views':
        ...
        order = 'total_views'
        classactiveremen = "active"
        request.session['classname'] = "classactiveremen"
    else:
        queryset = Article.objects.filter(status=3).order_by('-pub_time', '-id')
        order = 'normal'
        classactiveindex = "active"
        request.session['classname'] = "classactiveindex"
    ...
    context = {
                ...
                'category': category,
                'order': order,
                "classactiveindex": classactiveindex,
                "classactiveremen": classactiveremen,
              }
...
def CategoryList(request, pd):
    classactivecategory = "active"
    if not request.session.get('is_login', None):
        return redirect('login')
    request.session['classname'] = "classactivecategory"
    if request.GET.get('order') == 'total_views':
    ...
    page = request.GET.get('page')
    articles = paginator.get_page(page)
    category = Category.objects.all().order_by("id").annotate(classid=Value('', TextField()))
    for i in category:
        if i.id == pd:
            i.classid = "active"
            request.session['classcategory'] = category
    category1 = Category.objects.get(id=pd)
    # category = category.values_list(
    #     'id', 'name', 'classid'
    # )
    ...
    context = {
                ...
                'category1': category1,
                'order': order,
                'classactivecategory': classactivecategory
              }
...
class ArticleDetail(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.session['classcategory']:
            context['category'] = self.request.session['classcategory']
        if self.request.session['classname'] == "classactiveindex":
            context['classactiveindex'] = "active"
        if self.request.session['classname'] == "classactiveremen":
            context['classactiveremen'] = "active"
        if self.request.session['classname'] == "classactivecategory":
            context['classactivecategory'] = "active"
        if self.request.session['classname'] == "classactivemy":
            context['classactivemy'] = "active"
        # context['classactivemy'] = classactivemy  # 将表单对象传送到模板的数据中
        # print(context)
        return context
...
def article_create(request):
    ...
    # 如果用户请求获取数据
        ...
        # 赋值上下文
        context = {'article_post_form': article_post_form, 'category': category, "tag": tag, 'classactiveadd': classactiveadd}
        # 返回模板
...
def MyArticle(request):
    ...
    order = 'normal'
    request.session['classname'] = "classactivemy"
    # queryset = Article.objects.filter(status=1).order_by('-id')
    # 每页显示 1 篇文章
    paginator = Paginator(queryset, 5)
    # 获取 url 中的页码
...
def Tag_add(request):
    ...
    if request.method == "POST":
        ...
        #     result = '100'
        #     return HttpResponse(result)
            # return HttpResponse("标签内容不能为空，请重新填写。")
        if tag_name.isspace():
            result = '200'
            return HttpResponse(result)
            # return HttpResponse("标签内容不能为空格，请重新填写。")
        else:
            for tags in tag1:
                # print(tag_name, tags.name)
                # print(tags.name == tag_name, '1')
                if tags.name == tag_name:
                    result = '300'
                    return HttpResponse(result)
                    # return HttpResponse("标签内容重复，请重新填写。")
            tag.name = tag_name
            tag.save()
            tagID = Tag.objects.get(name=tag_name).id
            result = {"tagID": tagID, "tagName": tag_name}
            # print(result)
            # tag = serializers.serialize("json", Tag.objects.all())
